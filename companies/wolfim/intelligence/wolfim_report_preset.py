...
COLORS = {
    ...
    'gray_100': (232, 232, 227),    # #E8E8E3
    'gray_200': (208, 208, 203),    # #D0D0CB
    'gray_300': (160, 160, 155),    # #A0A09B
    'gray_400': (112, 112, 104),    # #707068
    'gray_500': (80, 80, 74),       # #50504A
    'green': (16, 185, 129),        # #10B981
    'whatsapp': (37, 211, 102),     # #25D366
    'red': (230, 30, 30),           # logo accent only
...
class WolfimReport(FPDF):
    ...
    def add_cover(self, title: str, subtitle: str, client: str, period: str, prepared_for: str | None = None, date_label: str | None = None):
        ...
        self.set_xy(16, 54)
        self.set_font('DISPLAY', 'B', 25.5)
        self.set_text_color(*self.color('black'))
        self.multi_cell(142, 10.5, title)
        self.ln(2)
        self.set_x(16)
        self.set_font('BODY', '', 9.4)
        self.set_text_color(*self.color('gray_500'))
        self.multi_cell(144, 5.2, subtitle)
        ...
        self.set_y(247)
        self.set_font('BODY', '', 7.7)
        self.set_text_color(*self.color('gray_400'))
        self.multi_cell(128, 4.7, 'Reporte preparado para convertir datos en decisiones comerciales concretas.')
        self.image(str(self.assets['logo_black']), x=151, y=247, w=43)
        self.set_y(270)
        self.set_x(16)
        self.set_font('MONO', '', 5.8)
        self.set_text_color(*self.color('gray_400'))
        self.cell(28, 4, 'BUENOS AIRES')
        self.set_fill_color(*self.color('green'))
        self.ellipse(45, 271.2, 1.8, 1.8, 'F')
        self.set_x(49)
    ...
    def meta_line(self, label: str, value: str):
        self.set_font('MONO', 'B', 6.3)
        self.set_text_color(*self.color('black'))
        self.cell(43, 5.5, label)
        self.set_font('BODY', '', 7.6)
        self.set_text_color(*self.color('gray_500'))
    ...
    def section(self, title: str, kicker: str | None = None):
        ...
        self.set_font('DISPLAY', 'B', 13.7)
        self.set_text_color(*self.color('black'))
        self.cell(0, 7.5, title, new_x='LMARGIN', new_y='NEXT')
        y = self.get_y()
        self.set_fill_color(*self.color('green'))
        self.ellipse(16, y - 1.2, 1.9, 1.9, 'F')
        self.set_draw_color(*self.color('gray_200'))
        self.set_line_width(0.18)
        self.line(21, y, 194, y)
        self.ln(4)

    def body(self, text: str, size: float = 8.6, color: str = 'gray_500'):
        self.set_font('BODY', '', size)
        self.set_text_color(*self.color(color))
        self.multi_cell(0, 4.85, text)
        self.ln(2)
    ...
    def callout(self, title: str, text: str, tone: str = 'green'):
        ...
        self.rect(x, y, 1.7, 25, 'F')
        self.set_xy(x + 6, y + 4.2)
        self.set_font('DISPLAY', 'B', 8.5)
        self.set_text_color(*self.color('black'))
        self.cell(0, 4.5, title, new_x='LMARGIN', new_y='NEXT')
        self.set_x(x + 6)
        self.set_font('BODY', '', 7.4)
        self.set_text_color(*self.color('gray_500'))
        self.multi_cell(w - 12, 4.1, text)
        self.set_y(y + 29)
    ...
    def metric_cards(self, items: Sequence[tuple[str, str, str]], columns: int = 3):
        ...
        for i, (label, value, note) in enumerate(items):
            ...
            self.set_fill_color(*self.color('green'))
            self.ellipse(x + 5, y + 4.1, 2.1, 2.1, 'F')
            self.set_xy(x + 5, y + 7.1)
            self.set_font('DISPLAY', 'B', 13.2)
            self.set_text_color(*self.color('black'))
            self.cell(w - 10, 5.8, value, new_x='LMARGIN', new_y='NEXT')
            self.set_xy(x + 5, y + 14.1)
            self.set_font('MONO', 'B', 5.7)
            self.set_text_color(*self.color('black'))
            self.multi_cell(w - 10, 3.1, label.upper())
            if note:
                self.set_xy(x + 5, y + 20.1)
                self.set_font('BODY', '', 5.8)
                self.set_text_color(*self.color('gray_400'))
    ...
    def table(self, headers: Sequence[str], rows: Sequence[Sequence[str]], widths: Sequence[float], font_size: float = 7.0, row_h: float = 9.0):
        ...
        self.set_font('MONO', 'B', font_size - 0.7)
        self.set_fill_color(*self.color('black'))
        self.set_text_color(*self.color('white'))
        for h, w in zip(headers, widths):
            self.cell(w, 6.7, str(h).upper(), border=0, fill=True)
        self.ln()
        fill = False
        for row in rows:
            ...
            for i, (val, w) in enumerate(zip(row, widths)):
                ...
                self.multi_cell(w, 4.15, str(val), border='B', fill=fill)
            self.set_y(y + row_h)
            self.set_draw_color(*self.color('gray_200'))
            fill = not fill
        self.ln(3)
    ...
    def bullets(self, items: Sequence[str]):
        self.set_font('BODY', '', 8.1)
        self.set_text_color(*self.color('gray_500'))
        for item in items:
            y = self.get_y() + 1.35
            self.set_fill_color(*self.color('green'))
            self.ellipse(18, y, 1.25, 1.25, 'F')
            self.set_x(22)
            self.multi_cell(170, 4.6, item)
        self.ln(2)
